...
def generaNumeroPseudoCasuale(x:int, y:int):
    ...
    if base % 2 == 0:
        return ((base * 42 + 12) * x - y) % 100 + 1 #Operazione inventata senza senso logico eccetto "% 100 + 1"
    else:
        return ((base * 21 + 24) * x - y) % 100 + 1 #Operazione inventata senza senso logico eccetto "% 100 + 1"
...
while sequenza[len(sequenza)-1] < num: # sequenza[len(sequenza)-1] uguale a scrivere sequenza[-1]
    ...
    if checkProssimo(prossimo, num) == True:
